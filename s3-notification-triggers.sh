...
bucket_name="project2-s3-notification-trigger"
lambda_func_name="project2-s3-lambda-notification-trigger-function"
role_name="project2-s3-lambda-sns"
email_address="bryanpedrazawfg2@gmail.com"

# Create IAM Role for the project
role_response=$(aws iam create-role --role-name project2-s3-lambda-sns --assume-role-policy-document '{
  "Version": "2012-10-17",
  "Statement": [{
...
aws lambda add-permission \
  --function-name "$lambda_func_name" \
  --statement-id "project2-s3-lambda-sns" \
...
topic_arn=$(aws sns create-topic --name project2-s3-lambda-sns --output json | jq -r '.TopicArn')
...
aws sns publish \
  ...
  --message "Hello World. i have my resume on the cloud visit here: www.devopsbybryan.com"